...
`make test` builds a real Material site with plugins enabled, under `--strict`.
...
| `run`     | Serve the docs in the current directory on :8000 |
...
`make lock` / `make lock-check` keep `requirements.lock` honest against
`requirements.txt`.
